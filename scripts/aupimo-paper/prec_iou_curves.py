...
_ = ax.legend(loc="upper left", title="Image index")
...
for iteridx, imgidx in enumerate(images_idxs):
    ...
    fig_imshow, ax_imshow = plt.subplots(1, 1, figsize=(6, 6), layout="constrained")
    _ = ax_imshow.imshow(img)
    _ = ax_imshow.imshow(asmap, cmap="jet", alpha=0.4)
    cs = ax_imshow.contour(masks[imgidx], levels=[0.5], colors="white", lw=1, ls="--")
    _ = ax.clabel(cs, cs.levels, inline=True, fmt={cs.levels[0]: "ground truth"}, fontsize=10)
    cs = ax_imshow.contour(asmap, threshs, lw=1, colors=["black", "black", "black"], labels=pivots)
    fmt = {l: f"{val:.0%}" for l, val in zip(cs.levels, prec_values, strict=False)}
    _ = ax_imshow.clabel(cs, cs.levels, inline=True, fmt=fmt, fontsize=10)
    pivots_str = ", ".join([f"{p:.0%} ({per_image_tpr_values_shared[imgidx, thidx]:.0%})" for p, thidx in zip(pivots, thresh_idxs)])
    _ = ax_imshow.set_title(f"Image {imgidx}. Contours at precision (corresponding recall):\n{pivots_str}")
    _ = ax_imshow.axis("off")
    
    ax.scatter(threshs, prec_values, s=50, c=ax.get_legend_handles_labels()[0][iteridx].get_color())
    
    fig_imshow.savefig(SAVEDIR / f"precision_contours_{imgidx}.pdf", bbox_inches="tight", pad_inches=1e-2)

fig.savefig(SAVEDIR / f"precision_curves.pdf", bbox_inches="tight", pad_inches=1e-2)
...
for iteridx, imgidx in enumerate(images_idxs):
    ...
    _ = ax.scatter(threshs_shared[pivots_thresh_idxs], iou_curve[pivots_thresh_idxs], marker="o", s=50, c=ax.get_legend_handles_labels()[0][iteridx].get_color())
    ...
    _ = ax.clabel(cs, cs.levels, inline=True, fmt={cs.levels[0]: "ground truth"}, fontsize=10)
    cs = ax_imshow.contour(asmap, threshs, lw=1, colors=["black", "black", "black"], labels=pivots)
    fmt = {l: f"{val:.0%}" for l, val in zip(cs.levels, iou_values, strict=False)}
    _ = ax_imshow.clabel(cs, cs.levels, inline=True, fmt=fmt, fontsize=10)
    # pivots_str = [f"{p:.0%} ({per_image_tpr_values_shared[imgidx, thidx]:.0%})" for p, thidx in zip(iou_values, all_pivots_thresh_idxs)]
    pivots_str = [f"{p:.0%}" for p, thidx in zip(iou_values, all_pivots_thresh_idxs)]
