    ...
    print(
        argstrs := [
            ...
            for arg in [
                ...
                "--asmaps ../../../data/experiments/benchmark/efficientad_wr101_s_ext/mvtec/tile/asmaps.pt",
...
def draw4(ax):
    ...
    iou = superpixel_oracle_selection['iou'] if superpixel_oracle_selection is not None else None
    _ = ax.annotate(
        f"IoU: {iou:.0%}" if iou is not None else "IoU: None",
        xy=(1, 0),
        xycoords="axes fraction",
        xytext=(-10, 10),
        textcoords="offset points",
...
def draw5(ax):
    _ = ax.imshow(img)
    _ = ax.imshow(asmap, cmap=_get_cmap_transparent_bad("jet"), alpha=0.45)
    cs_gt = ax.contour(
        mask,
        levels=[0.5],
        colors="black",
...
def draw6(ax):
    ...
    # relative_sobel = valid_asmap_sobel / np.abs(asmap)
    # relative_sobel[np.isnan(relative_sobel)] = 0
    # relative_sobel[asmap == 0] = 0

    # relative_sobel = valid_asmap_sobel / np.abs(asmap)
    # relative_sobel = np.log10(relative_sobel)
    # relative_sobel[np.isinf(relative_sobel)] = np.nan
    # relative_sobel[np.isnan(relative_sobel)] = np.nan

    relative_sobel = valid_asmap_sobel.flatten().argsort().argsort().reshape(valid_asmap_sobel.shape) / (valid_asmap_sobel.size - 1)

    _ = ax.imshow(
        relative_sobel,
        cmap=_get_cmap_transparent_bad("jet"),
        # alpha=0.45,
        # vmin=-4, vmax=4,
        vmin=0.8, vmax=1,
    )
    ...
    print(f"{np.nanmin(relative_sobel)=}, {np.nanmax(relative_sobel)=}")
    cs_gt = ax.contour(
        mask,
        levels=[0.5],
        colors="black",
